...
do
    local Window = Fluent:CreateWindow({
        Title = string.format("%s <b><i>%s</i></b>", string.format(MonthlyLabels[os.date("*t").month], "Nai-AimBot"), #Status > 0 and Status or "🗣️Naibell-God🗣️"),
        SubTitle = "By github.com/naibelgodd",
    ...
    Tabs.Aimbot:AddParagraph({
        Title = string.format("%s 🗣️Naibell-God🗣️", string.format(MonthlyLabels[os.date("*t").month], "Naibell-Recode")),
        Content = "Naibell-Recode-This\nhttps://github.com/naibelgodd"
    })
    ...
    Tabs.Bots:AddParagraph({
        Title = string.format("%s 🗣️Naibell-God🔥", string.format(MonthlyLabels[os.date("*t").month], "Recode-Aim")),
        Content = "Naibell-Recode-This\nhttps://github.com/naibelgodd"
    })
    ...
    PremiumChecksSection:AddParagraph({
        Title = string.format("%s 💫mua-cặc💫", string.format(MonthlyLabels[os.date("*t").month], "Recode-Aim")),
    ...
    if DEBUG or getfenv().Drawing and getfenv().Drawing.new then
        ...
        Tabs.Visuals:AddParagraph({
            Title = string.format("%s 🔥Naibell-God🔥", string.format(MonthlyLabels[os.date("*t").month], "Recode-Aim")),
            Content = "Naibell-Recode-This\nhttps://github.com/naibelgodd"
        })
    ...
    if getfenv().setclipboard then
        ...
        DiscordWikiSection:AddParagraph({
            Title = "https://github.com/naibelgodd/aim/main/naibell.lua",
    ...
    if UISettings.ShowWarnings then
        ...
        elseif ShowWarning then
            Window:Dialog({
                Title = "Warning",
                Content = string.format("Your Software does not support all the Features of %s 🔥Naibell-God🔥!", string.format(MonthlyLabels[os.date("*t").month], "Recode-Aim")),
                Buttons = {
                    {
                        Title = "Confirm"
                    }
            ...
            Window:Dialog({
                Title = string.format("%s 💫mua cặc💫", string.format(MonthlyLabels[os.date("*t").month], "Open Aimbot")),
                Content = "✨Sybau💔✨ – Contact @ttwiz_z via Discord to buy",
                Buttons = {
                    {
                        Title = "Confirm"
                    }
...
local function Notify(Message)
    if Fluent and typeof(Message) == "string" then
        Fluent:Notify({
            Title = string.format("%s 🔥Naibell-God🔥", string.format(MonthlyLabels[os.date("*t").month], "Recode-Aim")),
            Content = Message,
            SubContent = "By https://github.com/naibelgodd",
            Duration = 1.5
        })
    end
end
